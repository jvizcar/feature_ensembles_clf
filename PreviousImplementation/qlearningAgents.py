...
class QLearningAgent(ReinforcementAgent):
    ...
    def __init__(self, **args):
        ...
        "*** YOUR CODE HERE ***"
        # Dictionary of state: actions (util.Counters)
        self.Qvalues = {}
        "*** END YOUR CODE HERE ***"
    ...
    def computeValueFromQValues(self, state):
        ...
        # If no legal actions -> 0
        if len(actions) == 0:
            return 0.0

        # Best action (maximizes Q-Value)
        max_action_Q = Q_Values[ Q_Values.argMax() ]
        return max_action_Q
        "*** END YOUR CODE HERE ***"
        util.raiseNotDefined()
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        next_QVs = util.Counter()
        a_prime = self.getLegalActions(nextState)
        for a in a_prime:
            next_QVs[a] = self.getQValue(nextState, a)

        # Sample = R(s,a,s') + gamma * max_a'( Q(s',a') )
        sample = reward + self.discount * next_QVs[ next_QVs.argMax() ]
        alpha = self.alpha
        Q = self.getQValue(state, action)
        # Q(s,a) = (1-alpha) Q(s,a) + alpha sample
        Q_Value = (1-alpha) * Q + alpha * sample
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        # List of a':
        a_prime_values = self.getLegalActions(nextState)
